...
青龙环境变量  jetour_aaa={"token":"xxxxxxxxxxxxx"}
随便找到url里面有access_token的就是这个token
*/

const $ = new Env("捷途汽车");
const _key = 'jetour_aaa';
...
async function tasks() {
    url = `https://mobile-consumer.jetour.com.cn/web/task/tasks/event-start?access_token=${$.huihui.token}`;
    body = '{"eventCode":"SJ50001"}';
    headers = {
        "Content-Type": "application/json",
        "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 ios/1.0.0"
    }
    const {status, message} = await httpRequest({url, body, headers});
    pushMsg(status === 200 ? '签到成功' : message);
}

//取ck
function getCk() {
    if ($request && $request.method != 'OPTIONS') {
        const token = getParameterByName('access_token', $request.url)
